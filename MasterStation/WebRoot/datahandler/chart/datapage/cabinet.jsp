...
<html>
							 ...
							 <s:else>
								...
								<td>
									<span><input class="toolbarButton" type="button"
											value="确定" onclick="parent.Chart.selectedDevice(3);return false;"/> </span>
								</td>
							</tr>
				</table>
 ...
 onmouseover="javascript:this.bgColor='#f5fafe'">
								...
								<td width="5%">
									<input type="radio" name="cabinet" value="<s:property value="#cabinet.cabId"/>" onclick="selectDevice()" data="<s:property value="#cabinet.line.name"/><s:property value="#cabinet.cabNumber"/><s:property value="#cabinet.cabType.value"/>"/>
									<s:property value="#status.count+(pageList*(page-1))"/>
								</td>
								<td width="20%">
									
									<s:property value="#cabinet.line.name"/>
								</td>
								<td width="20%">
									
									<s:property value="#cabinet.cabNumber"/>
									<s:property value="#cabinet.cabType.value"/>
								</td>
								...
								<td width="20%">
									<s:date name="#cabinet.createTime" format="yyyy-MM-dd" />
									<s:date name="#cabinet.createTime" format="HH:mm:ss" />
								</td>
							</tr>
						</s:iterator>
					</table>
			...
			<s:iterator value="pageBean">
				<table border="0" cellpadding="0" cellspacing="0">
					<tbody>
						<tr>
							<td>
								<select class="pagination-page-list" name="pageList" onchange="window.location='listCabinetComAction.action?pageList='+this.options[this.options.selectedIndex].value">
							...
							<td>
								...
								<a href="listCabinetComAction.action?page=1"
							...
							<td>
								<s:if test="CurrentPage>1">
								<a href="listCabinetComAction.action?page=${CurrentPage-1 }"
							...
							<td>
								...
								<s:else>
								<a href="listCabinetComAction.action?page=${TotalPage}"
							...
							<td>
								<a href="listCabinetComAction.action?page=${CurrentPage}" class="pagination-load-btn p-plain"><span
		...
		<script>
			...
			function selectDevice(){
				var value = $('input[name="cabinet"]:checked').val();
				var data = $('input[name="cabinet"]:checked').attr("data");
				//alert(data);	
				$(('#selectedDevice'),parent.document).html("<option value='"+value+"'>"+data+"</option>");
				//alert($('input[name="device"]:checked').val());	
			}	
			$('input[name="cabinet"]').each(function(){
				var cabId = $(('#selectedDevice'),parent.document).val();
				//alert($(this).val());
				if(cabId==$(this).val()){
