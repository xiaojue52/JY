...
public class HistoryAction extends ActionSupport {
	...
	public String createSql(){
		...
		if (queryDetector==null||queryDetector.length()==0)
			queryDetector = "%";
		hql =hql+ "history.detector.device.cabinet.line.name like '%"+queryLine+"%' and "+
			  "history.detector.device.cabinet.cabNumber like '%"+queryNumber+"%' and "+
			  "history.detector.device.name like '%"+queryDevice+"%' and "+
			  "history.detector.name like '%"+queryDetector+"%' and "+
			  "history.detector.device.cabinet.cabType.value like '%"+queryType+"%' and "+
			  "history.collectDate>= TO_DATE('" + queryStartDate + "','YYYY-MM-DD') and "+"history.collectDate <= TO_DATE('"+queryEndDate+"','YYYY-MM-DD') and "+
			  "history.detector.device.cabinet.user.username like '%"+queryUser+"%' ORDER BY id DESC";
		return hql;
	}
